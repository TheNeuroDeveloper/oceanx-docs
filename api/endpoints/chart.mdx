...
| `mode` | `price` | `price` for USD prices, `mc` for market cap on the same axis. |
...
| `o` `h` `l` `c` | Open, high, low, close. USD prices, or market cap under `mode=mc`. |
| `v` | Volume traded in the bucket, in USD. |
...
**All prices are USD.** There is no SOL-denominated mode on this endpoint today.
