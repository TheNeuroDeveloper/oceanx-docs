...
The OceanX Developer API lets you track Solana wallets, quote swaps, and receive the same data and live feed the OceanX app itself uses — programmatically. **v1** covers **wallet tracking** (add and remove wallets, read tracker state, pull recent trades, open a realtime WebSocket) and **spot quotes** (executable quotes off OceanX's own router, plus an exit checker that tells you whether a whole position can actually be sold).
